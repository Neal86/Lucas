...
'Computer Node':'电脑节点','Node ID':'Node ID','Effective permission':'实际权限','Permission':'权限','Allowed folders':'允许的文件夹','Node log':'Node 日志','Refresh logs':'刷新日志','Close':'关闭','View':'查看',
...
'Add AI':'添加 AI','Add AI connection':'添加 AI 连接','Copy MCP URL':'复制 MCP URL','Registered AI / MCP clients':'已注册的 AI / MCP 客户端','Ready':'就绪','Refresh':'刷新','Expand all':'全部展开','Collapse all':'全部折叠','Subtasks':'子任务','Recorded time':'记录时长','Filter':'筛选','Action filter':'操作筛选','Target':'目标','Authentication provider:':'认证方式：',
...
function nodeRowHtml(n){return `<tr data-node-key="${encodeURIComponent(n.node_id)}"><td><b>${esc(n.name||n.node_id)}</b><div class="muted">${esc(n.node_id)}</div></td><td><span class="badge ${n.online?'online':'offline'}">${n.online?'Online':'Offline'}</span></td><td>${esc(accessMode(n))}</td><td class="path">${esc((n.allowed_roots||[]).join('\n')||'—')}</td><td>${esc(timefmt(n.last_seen||n.updated_at))}</td><td><button class="btn secondary" onclick="openNodeModal('${encodeURIComponent(n.node_id)}')">View</button></td></tr>`}
