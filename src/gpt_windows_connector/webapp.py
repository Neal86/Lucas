...
async function saveNodeName(){if(!managedNodeId)return;const name=manageNodeNameInput.value.trim();if(!name){toast('Computer name is required');return}await api('/api/nodes/'+encodeURIComponent(managedNodeId)+'/config',{method:'PUT',body:JSON.stringify({name})});toast('Computer name updated');await refresh();const n=state.nodes.find(x=>x.node_id===managedNodeId);if(n)manageNodeNameInput.value=n.name||n.node_id}
...
async def api_node_config(request: Request):
    ...
    try:
        await gateway.auth_store.update_name(node_id, user.id, name)
