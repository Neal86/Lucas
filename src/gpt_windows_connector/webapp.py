...
.brand img{display:block;width:min(250px,100%);height:auto;margin-bottom:8px}.logo img{display:block;width:184px;height:auto;background:transparent;border-radius:0;padding:0;filter:brightness(0) invert(1)!important}.landing-logo img{display:block;height:44px;width:auto;background:transparent;border-radius:0;padding:0;filter:brightness(0) invert(1)}.core-ring img{width:66px;height:66px;object-fit:contain;background:transparent;border-radius:0;padding:0;filter:brightness(0) invert(1)}.landing-footer .landing-logo img{height:38px}@media(max-width:850px){.logo img{width:150px}}
</style>
...
<section id="landing" class="landing">
  ...
  <section id="how" class="how-section"><div class="section-kicker">THREE STEPS</div><h2>From AI to action.</h2><div class="steps"><div><span>1</span><h3>Connect a computer</h3><p>Install Lucas Node, enter its Node ID and local Connection Code, then approve the account on that computer.</p></div><i>→</i><div><span>2</span><h3>Add Lucas MCP</h3><p>Use one MCP endpoint with any compatible AI or agent.</p></div><i>→</i><div><span>3</span><h3>Start working</h3><p>Your AI can now use the tools you've explicitly allowed.</p></div></div></section>
...
<div id="aiModal" class="modal-backdrop hidden"><div class="modal"><h3>Add AI connection</h3><p class="muted">Lucas accepts multiple independent OAuth-capable MCP clients. Choose a client for setup guidance, then add the same MCP URL in that client.</p><div class="field"><label>AI / MCP client</label><select id="aiClientType" onchange="updateAiHelp()"><option>ChatGPT</option><option>Claude</option><option>Gemini</option><option>Cursor</option><option>Codex</option><option>Claude Code</option><option>Other MCP client</option></select></div><div class="field"><label>MCP Server URL</label><input id="aiModalUrl" class="input" readonly value="https://lucasmcp.com/mcp"></div><div id="aiHelp" class="card muted"></div><div class="actions"><button class="btn secondary" onclick="closeModal('aiModal')">Close</button><button class="btn primary" onclick="copyMcpUrl()">Copy MCP URL</button></div></div></div><div id="aiManageModal" class="modal-backdrop hidden"><div class="modal"><h3>Edit AI connection</h3><div class="field"><label>Name</label><input id="aiManageName" class="input" maxlength="100"></div><div class="field"><label>Note</label><textarea id="aiManageNote" class="input" maxlength="500" rows="4"></textarea></div><div class="actions"><button class="btn secondary" onclick="closeModal('aiManageModal')">Close</button><button class="btn primary" onclick="saveAiConnection()">Save</button></div></div></div><div id="connectModal" class="modal-backdrop hidden"><div class="modal"><h3>Connect to a computer</h3><p class="muted">Enter the Node ID and 8-digit Connection Code shown in Lucas Settings on the target computer. After the code is verified, that computer must still approve this Lucas account locally.</p><div class="field"><label>Node ID</label><input id="connectNodeId" class="input" autocomplete="off" placeholder="computer-xxxxxxxxxxxx"></div><div class="field"><label>Connection Code</label><input id="connectCode" class="input" autocomplete="off" inputmode="numeric" maxlength="8" placeholder="12345678"></div><div id="connectResult" class="card hidden"></div><div class="actions"><button class="btn secondary" onclick="closeModal('connectModal')">Close</button><button class="btn primary" onclick="requestNodeAccess()">Send access request</button></div></div></div>
<div id="toast" class="toast hidden"></div>
<script>
const WEB_LANG=((navigator.language||navigator.userLanguage||'en').toLowerCase().startsWith('zh'))?'zh':'en';
document.documentElement.lang=WEB_LANG==='zh'?'zh-CN':'en';
const WEB_ZH={
'Capabilities':'功能','Security':'安全','How it works':'工作原理','Sign in':'登录','Dashboard':'控制台',
'Let your AI leave':'让你的 AI 走出','the chat box.':'聊天框。','Connect your computer':'连接电脑','See how it works ↓':'查看工作原理 ↓',
'Model agnostic':'不限模型','Cross-platform':'跨平台','Token-free execution':'无额外执行 Token',
'Terminal & Code':'终端与代码','Files & Projects':'文件与项目','Browser':'浏览器','Computer Use':'电脑操作','Git':'Git','Remote Access':'远程访问',
'Your computer.':'你的电脑。','Your boundaries.':'你的边界。','Project-scoped access':'项目范围访问','Local permission control':'本地权限控制','OAuth-secured MCP':'OAuth 安全 MCP','Activity visibility':'操作记录可见',
'THREE STEPS':'三个步骤','From AI to action.':'从 AI 到实际执行。','Connect a computer':'连接电脑','Add Lucas MCP':'添加 Lucas MCP','Start working':'开始工作',
'Any AI.':'任何 AI。','Any computer.':'任何电脑。','Get started with Lucas':'开始使用 Lucas',
'Email':'邮箱','Password':'密码','Continue with Google':'使用 Google 继续','No account?':'没有账号？','Create one':'创建账号','Name':'姓名','Create account':'创建账号','Sign up with Google':'使用 Google 注册','Already registered?':'已有账号？','Verification code':'验证码','Verify email':'验证邮箱','Resend code':'重新发送',
'Computer Nodes':'电脑节点','AI Connections':'AI 连接','Task Runs':'任务记录','Activity Logs':'活动日志','Account & Security':'账号与安全','Sign out':'退出登录','Authorized computers':'已授权电脑','Online computers':'在线电脑','AI clients':'AI 客户端','Recent activity':'最近活动',
'Windows':'Windows','Available':'可用','Download Lucas Node':'下载 Lucas Node','Connect by Node ID':'通过 Node ID 连接','Coming Soon':'即将推出','macOS':'macOS','Linux':'Linux',
'Computer Node':'电脑节点','Computer name':'电脑名称','Node ID':'Node ID','Effective permission':'实际权限','Permission':'权限','Allowed folders':'允许的文件夹','Node log':'Node 日志','Refresh logs':'刷新日志','Close':'关闭','View':'查看',
'Connect to a computer':'连接到电脑','Requested permission':'申请权限','Read':'读取','Operate':'操作','Admin':'管理员','Send access request':'发送访问申请',
'Computer':'电脑','Status':'状态','Last seen':'最后在线','Online':'在线','Offline':'离线','No authorized computers yet. Use Connect by Node ID to request access.':'暂无已授权电脑。请通过 Node ID 发起访问申请。',
'Add AI':'添加 AI','Add AI connection':'添加 AI 连接','Copy MCP URL':'复制 MCP URL','Registered AI / MCP clients':'已注册的 AI / MCP 客户端','Edit AI connection':'编辑 AI 连接','Name':'名称','Note':'备注','Edit':'编辑','Delete':'删除','Save':'保存','Ready':'就绪','Refresh':'刷新','Expand all':'全部展开','Collapse all':'全部折叠','Subtasks':'子任务','Recorded time':'记录时长','Filter':'筛选','Action filter':'操作筛选','Target':'目标','Authentication provider:':'认证方式：',
'Users':'用户','Usage':'用量','Nodes':'节点','Operations':'操作','Subscriptions':'订阅','System':'系统','Loading…':'加载中…','Search':'搜索','Role':'角色','Plan':'套餐','Save account':'保存账号','Recent operations':'最近操作','Time':'时间','Action':'操作','Details':'详情','User':'用户',
'Account access and security settings are controlled locally on the target computer.':'账号访问和安全权限由目标电脑本地控制。',
'Enter the Node ID shown in Lucas Settings on the target computer. The target computer must approve this Lucas account locally before any operation is allowed.':'输入目标电脑 Lucas 设置中显示的 Node ID。任何操作前，都必须由目标电脑在本地批准此 Lucas 账号。',
'No pairing code required':'无需配对码','Waiting for approval on the target computer…':'正在等待目标电脑批准…','Access approved and saved on the target computer.':'访问已批准并保存在目标电脑。','Access approved for this session.':'本次会话访问已批准。','Access was not approved on the target computer.':'目标电脑未批准访问。','Node ID is required':'请输入 Node ID',
...
const viewPaths={dashboard:'/dashboard',nodes:'/nodes',ai:'/ai-connections',tasks:'/task-runs',logs:'/logs',account:'/account',admin:'/admin'};
...
function accessMode(n){return ({request_approval:'Ask for approval',auto_approve:'Auto-approve safe actions',full_access:'Full Access',custom:'Custom'})[n.preset]||'Ask for approval'}
function nodeRowHtml(n){return `<tr data-node-key="${encodeURIComponent(n.node_id)}"><td><b>${esc(n.display_name||n.name||n.node_id)}</b><div class="muted">${esc(n.node_id)}</div></td><td><span class="badge ${n.online?'online':'offline'}">${n.online?'Online':'Offline'}</span></td><td>${esc(accessMode(n))}</td><td class="path">${esc((n.allowed_roots||[]).join('\n')||'—')}</td><td>${esc(timefmt(n.last_seen||n.updated_at))}</td><td><button class="btn secondary" onclick="openNodeModal('${encodeURIComponent(n.node_id)}')">View</button></td></tr>`}
function renderNodes(){if(!state.nodes.length){nodeTable.innerHTML='<div class="card empty">No authorized computers yet. Use Connect computer to request access.</div>';updateNodeMetrics();return}nodeTable.innerHTML='<table class="table"><thead><tr><th>Computer</th><th>Status</th><th>Access mode</th><th>Allowed folders</th><th>Last seen</th><th></th></tr></thead><tbody>'+state.nodes.map(nodeRowHtml).join('')+'</tbody></table>';updateNodeMetrics()}
...
async function loadTaskRuns(){const d=await api('/api/task-runs?limit=100'),runs=d.runs||[];const total=runs.reduce((a,r)=>a+(Number(r.duration_ms)||0),0),steps=runs.reduce((a,r)=>a+(r.steps||[]).length,0);taskRunSummary.innerHTML=`<div class="card"><div class="muted">Task runs</div><div class="metric">${runs.length}</div></div><div class="card"><div class="muted">Subtasks</div><div class="metric">${steps}</div></div><div class="card"><div class="muted">Recorded time</div><div class="metric" style="font-size:22px">${durationfmt(total)}</div></div>`;if(!runs.length){taskRunTable.innerHTML='<div class="card empty">No Task Runs recorded yet.</div>';return}const controls='<div class="task-run-controls"><button class="btn secondary" onclick="setAllTaskRuns(true)">Expand all</button><button class="btn secondary" onclick="setAllTaskRuns(false)">Collapse all</button></div>';taskRunTable.innerHTML=controls+runs.map(r=>{const expanded=r.status==='running',count=(r.steps||[]).length;return `<div class="card task-run-card ${expanded?'expanded':''}" role="button" tabindex="0" aria-expanded="${expanded?'true':'false'}" onclick="toggleTaskRun(this,event)" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();toggleTaskRun(this,event)}"><div class="task-run-head"><div class="task-run-title"><span class="task-run-chevron">⌄</span><div><b>${esc(r.title)}</b><div class="muted">${esc(r.node_id)} · ${esc(timefmt(r.started_at))}</div></div></div><div class="task-run-right"><div class="task-run-meta">${count} ${count===1?'subtask':'subtasks'}</div><div>${taskStatusBadge(r.status)}<div class="task-run-duration">${esc(durationfmt(r.duration_ms))}</div></div></div></div><div class="task-run-details"><table class="table"><tbody>${(r.steps||[]).map(x=>`<tr><td style="width:35%"><b>${esc(x.action)}</b></td><td>${esc(x.status)}</td><td style="text-align:right">${esc(durationfmt(x.duration_ms))}</td></tr>`).join('')}</tbody></table></div></div>`}).join('')}
...
function openConnectModal(){connectNodeId.value='';connectCode.value='';connectResult.textContent='';connectResult.classList.add('hidden');connectModal.classList.remove('hidden');setTimeout(()=>connectNodeId.focus(),50)}
