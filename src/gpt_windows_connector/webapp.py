...
<section id="landing" class="landing">
  ...
  <section id="how" class="how-section"><div class="section-kicker">THREE STEPS</div><h2>From AI to action.</h2><div class="steps"><div><span>1</span><h3>Connect a computer</h3><p>Install Lucas Node, use its Node ID, and approve account access locally.</p></div><i>→</i><div><span>2</span><h3>Add Lucas MCP</h3><p>Use one MCP endpoint with any compatible AI or agent.</p></div><i>→</i><div><span>3</span><h3>Start working</h3><p>Your AI can now use the tools you've explicitly allowed.</p></div></div></section>
...
function renderNodes(){if(!state.nodes.length){nodeTable.innerHTML='<div class="card empty">No authorized computers yet. Use Connect by Node ID to request access.</div>';updateNodeMetrics();return}nodeTable.innerHTML='<table class="table"><thead><tr><th>Computer</th><th>Status</th><th>Permission</th><th>Allowed folders</th><th>Last seen</th><th></th></tr></thead><tbody>'+state.nodes.map(nodeRowHtml).join('')+'</tbody></table>';updateNodeMetrics()}
...
function openConnectModal(){connectNodeId.value='';connectPermission.value='operate';connectResult.textContent='';connectResult.classList.add('hidden');connectModal.classList.remove('hidden');setTimeout(()=>connectNodeId.focus(),50)}
async function requestNodeAccess(){const node_id=connectNodeId.value.trim();if(!node_id){toast('Node ID is required');return}connectResult.textContent='Waiting for approval on the target computer…';connectResult.classList.remove('hidden');try{const d=await api('/api/nodes/request-access',{method:'POST',body:JSON.stringify({node_id,requested_permission:connectPermission.value})});if(d.authorized){connectResult.textContent=d.decision==='always'?'Access approved and saved on the target computer.':'Access approved for this session.';await refresh()}else{connectResult.textContent='Access was not approved on the target computer.'}}catch(e){connectResult.textContent=e.message}}
...
async def api_request_node_access(request: Request):
    ...
    requested_permission = str(body.get("requested_permission") or "operate").strip().lower()
    if not node_id:
    ...
    try:
        ...
        result = await gateway.registry.rpc(node_id, user.id, "access.request", {"requested_permission": requested_permission}, actor=gateway._actor(user), timeout=180.0)
        gateway.auth.audit(user.id, "node.access_request", node_id, {"requested_permission": requested_permission, "authorized": bool(isinstance(result, dict) and result.get("authorized"))})
