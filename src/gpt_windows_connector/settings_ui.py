...
from .local_account import LocalAccountClient
from .settings_account_ui import build_account_page
from .settings_browser_ui import build_browser_page
...
def configure_gui(existing: dict[str, object]) -> dict[str, object] | None:
    ...
    NAV_EN = {"常规":"General","账号与插件":"Account & Integrations","浏览器":"Browser Profiles","安全":"Security","用户与权限":"Users & Permissions","文件访问":"File Access","网络":"Network","任务记录":"Task History","日志":"Logs","系统访问":"System Access"}
    C = {
    ...
    account_client=LocalAccountClient(gateway.get(),ACCOUNT_FILE,PLUGIN_CACHE_FILE)
    pages["账号与插件"]=build_account_page(tk=tk,parent=page_host,colors=C,font=FONT,client=account_client,device_id=node_id.get(),translate=T)
    pages["浏览器"]=build_browser_page(tk=tk,parent=page_host,colors=C,font=FONT,translate=T)
    ...
    desc={"常规":"连接身份与此电脑的 Lucas 基础配置。","账号与插件":"登录 Lucas 账号并同步各设备的插件和集成；本地权限不会跟随同步。","浏览器":"管理 Browser Bridge、浏览器 Profile、Agent 映射与本机浏览器身份隔离。","安全":"控制 AI 在这台电脑上可以执行的操作。安全设置只能在本机修改。","用户与权限":"管理哪些 Lucas 用户可以操作此电脑，以及每个用户的权限和允许目录。","文件访问":"使用 Allowed Folders 建立文件与工作区的硬边界。","网络":"控制互联网、局域网、域名与后台网络请求。","任务记录":"查看本机 Lucas 大任务与小任务执行时间。","日志":"查看本机 Node 实时日志，用于排查连接、认证和重连问题。","系统访问":"查看 Lucas 与 Windows 管理员权限的实际状态。"}
    def show_page(name):
    ...
    for name in ("常规","账号与插件","浏览器","安全","用户与权限","文件访问","网络","任务记录","日志","系统访问"):
