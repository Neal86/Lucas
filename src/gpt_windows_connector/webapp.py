...
<section id="landing" class="landing">
  <nav class="landing-nav"><div class="landing-logo"><span class="logo-mark">L</span>Lucas</div><div class="landing-links"><a href="#capabilities">Capabilities</a><a href="#security">Security</a><a href="#how">How it works</a></div><button class="landing-signin" onclick="openAuth()">Sign in <span>→</span></button></nav>
...
function openNodeModal(id){managedNodeId=decodeURIComponent(id);const n=state.nodes.find(x=>x.node_id===managedNodeId);if(!n)return;manageNodeNameText.textContent=n.name||n.node_id;managePermissionText.textContent=({read:'Safe · Read-only',operate:'Standard · Operate',admin:'Full Access · Admin'})[n.permission_level]||n.permission_level||'Standard';manageRootsText.textContent=(n.allowed_roots||[]).join('\n')||'—';manageLogs.textContent='Click Refresh logs to load the latest local Lucas Node log.';nodeModal.classList.remove('hidden')}
...
async function unpairNode(){if(!managedNodeId||!confirm('Unpair this computer? It will need a new pairing code to reconnect.'))return;await api('/api/nodes/'+encodeURIComponent(managedNodeId),{method:'DELETE'});closeModal('nodeModal');toast('Computer node unpaired');await refresh()}
...
async def api_node_config(request: Request):
    ...
    return JSONResponse({"error": "Security settings are local-only. Open Lucas Settings from the Windows tray."}, status_code=403)
    body = await request.json()
