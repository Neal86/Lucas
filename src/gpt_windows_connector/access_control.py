...
def _apply_safety_floor(policy: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in _SAFETY_GATES: effective[key]=_stricter_decision(effective.get(key),"always_ask","always_ask")
    effective.setdefault("background_control","allow"); return effective
...
def intersect_security(node_security: dict[str, Any] | None, user_security: dict[str, Any] | None) -> dict[str, Any]:
    ...
    node={**DEFAULT_SECURITY,**(node_security or {})}; user={**DEFAULT_SECURITY,**(user_security or {})}; node_policy={**DEFAULT_SECURITY["approval_policy"],**dict(node.get("approval_policy") or {})}; user_policy={**DEFAULT_SECURITY["approval_policy"],**dict(user.get("approval_policy") or {})}
...
def normalize_preset(value: str | None) -> str:
    value=str(value or "").strip(); return value if value in ACCESS_PRESETS else "request_approval"

def clamp_roots(requested: list[str] | tuple[str,...],node_roots: list[str] | tuple[str,...]) -> list[str]:
    allowed=[Path(value).expanduser().resolve() for value in node_roots]; result=[]
    for raw in requested:
        try: candidate=Path(raw).expanduser().resolve()
...
class LocalAccessStore:
    ...
    def load(self)->dict[str,Any]:
        try:
            ...
            if not isinstance(data,dict): return {"version":1,"users":{}}
        except (OSError,json.JSONDecodeError): return {"version":1,"users":{}}
        if not isinstance(data.get("users"),dict): data["users"]={}
        data.setdefault("version",1); return data
    def save(self,data:dict[str,Any])->None:
        self.path.parent.mkdir(parents=True,exist_ok=True); temporary=self.path.with_name(f"{self.path.name}.{os.getpid()}.tmp"); temporary.write_text(json.dumps(data,ensure_ascii=False,indent=2),encoding="utf-8"); temporary.replace(self.path)
    def list_users(self)->list[dict[str,Any]]:
        out=[]
        for user_id,record in self.load().get("users",{}).items():
            if isinstance(record,dict): out.append({"user_id":str(user_id),**record})
        return sorted(out,key=lambda item:str(item.get("name") or item.get("email") or item.get("user_id")).lower())
    def get(self,user_id:str)->dict[str,Any]|None:
        record=self.load().get("users",{}).get(str(user_id)); return dict(record) if isinstance(record,dict) else None
    def upsert(self,actor:dict[str,Any],preset:str,allowed_roots:list[str],*,security:dict[str,Any]|None=None,enabled:bool=True)->dict[str,Any]:
        ...
        data=self.load(); users=data.setdefault("users",{}); previous=users.get(user_id) if isinstance(users.get(user_id),dict) else {}; now=time.time(); normalized=normalize_preset(preset)
        record={"email":str(actor.get("email") or previous.get("email") or ""),"name":str(actor.get("name") or previous.get("name") or ""),"preset":normalized,"security":dict(security or preset_security(normalized)),"allowed_roots":list(dict.fromkeys(str(root) for root in allowed_roots if str(root).strip())),"enabled":bool(enabled),"approved_at":float(previous.get("approved_at") or now),"updated_at":now,"last_access":float(previous.get("last_access") or 0)}
        users[user_id]=record; pending=data.setdefault("pending",{}); pending.pop(user_id,None) if isinstance(pending,dict) else None; self.save(data); return {"user_id":user_id,**record}
    def list_pending(self)->list[dict[str,Any]]:
        pending=self.load().get("pending",{}); out=[]
        if not isinstance(pending,dict): return out
        for user_id,record in pending.items():
            if isinstance(record,dict): out.append({"user_id":str(user_id),**record,"_pending":True})
    ...
    def add_pending(self,actor:dict[str,Any])->dict[str,Any]:
        user_id=str(actor.get("user_id") or actor.get("id") or "").strip()
        if not user_id: raise ValueError("user_id is required")
        data=self.load(); pending=data.setdefault("pending",{}); now=time.time(); previous=pending.get(user_id) if isinstance(pending.get(user_id),dict) else {}; record={"email":str(actor.get("email") or previous.get("email") or ""),"name":str(actor.get("name") or previous.get("name") or ""),"requested_at":float(previous.get("requested_at") or now),"updated_at":now}; pending[user_id]=record; self.save(data); return {"user_id":user_id,**record,"_pending":True}
    def remove_pending(self,user_id:str)->bool:
        data=self.load(); pending=data.setdefault("pending",{}); removed=pending.pop(str(user_id),None) is not None
        if removed:self.save(data)
        return removed
    def remove(self,user_id:str)->bool:
        data=self.load(); users=data.setdefault("users",{}); removed=users.pop(str(user_id),None) is not None
        if removed:self.save(data)
        return removed
    def prune_root(self,removed_root:str)->int:
        try: target=Path(removed_root).expanduser().resolve()
        except Exception:return 0
        data=self.load(); users=data.setdefault("users",{}); changed=0
        for record in users.values():
            if not isinstance(record,dict):continue
            kept=[]; removed=False
            for raw in record.get("allowed_roots") or []:
                try:candidate=Path(str(raw)).expanduser().resolve()
                except Exception:candidate=Path(str(raw))
                if candidate==target or target in candidate.parents:removed=True;continue
                kept.append(str(raw))
            if removed:record["allowed_roots"]=kept;record["updated_at"]=time.time();changed+=1
        if changed:self.save(data)
        return changed
    def touch(self,user_id:str)->None:
        data=self.load(); record=data.setdefault("users",{}).get(str(user_id))
        if isinstance(record,dict):record["last_access"]=time.time();self.save(data)
    def effective(self,user_id:str,node_roots:list[str]|tuple[str,...])->dict[str,Any]|None:
        record=self.get(user_id)
        if not record or not bool(record.get("enabled",True)):return None
        roots=clamp_roots(list(record.get("allowed_roots") or []),list(node_roots))
        if not roots:return None
        preset=normalize_preset(record.get("preset") or ("full_access" if record.get("permission_level")=="admin" else "request_approval")); security=record.get("security") if isinstance(record.get("security"),dict) else preset_security(preset); security={**security,"approval_policy":_apply_safety_floor(dict(security.get("approval_policy") or {}))}; clean=dict(record); clean.pop("permission_level",None)
