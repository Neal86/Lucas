...
<div id="app" class="shell hidden"><aside class="side"><div class="logo">Lucas</div><button class="nav active" data-view="dashboard" onclick="view('dashboard',this)">Dashboard</button><button class="nav" data-view="projects" onclick="view('projects',this)">Projects</button><button class="nav" data-view="nodes" onclick="view('nodes',this)">Windows Nodes</button><button class="nav" data-view="ai" onclick="view('ai',this)">AI Connections</button><button class="nav" data-view="logs" onclick="view('logs',this)">Activity Logs</button><button class="nav" data-view="account" onclick="view('account',this)">Account & Security</button><div class="userbox"><div id="userName"></div><small id="userEmail"></small><div style="margin-top:10px"><button class="btn secondary" onclick="logout()">Sign out</button></div></div></aside><main class="main"><div id="dashboard" class="view"><div class="top"><div><h2>Dashboard</h2><p class="muted">Your projects, computers, and recent activity.</p></div><button class="btn primary" onclick="openProjectModal()">New project</button></div><div class="grid"><div class="card"><div class="muted">Projects</div><div id="metricProjects" class="metric">0</div></div><div class="card"><div class="muted">Online Windows nodes</div><div id="metricNodes" class="metric">0</div></div><div class="card"><div class="muted">Recent actions</div><div id="metricLogs" class="metric">0</div></div></div><div class="card" style="margin-top:16px"><h3>Recent activity</h3><div id="recentLogs"></div></div></div><div id="projects" class="view hidden"><div class="top"><div><h2>Projects</h2><p class="muted">Each project is bound to one Windows node and one allowed folder.</p></div><button class="btn primary" onclick="openProjectModal()">New project</button></div><div id="projectTable"></div></div><div id="nodes" class="view hidden"><div class="top"><div><h2>Windows Nodes</h2><p class="muted">Download the Lucas pairing script, generate a code, and connect this Windows PC.</p></div><div style="display:flex;gap:8px"><a class="btn secondary" style="text-decoration:none" href="/download/Lucas-Node.bat">Download Lucas Node</a><button class="btn primary" onclick="openPairModal()">Pair computer</button></div></div><div id="nodeTable"></div></div><div id="ai" class="view hidden"><div class="top"><div><h2>AI Connections</h2><p class="muted">Connect ChatGPT and other MCP clients to Lucas securely with OAuth.</p></div></div><div class="card"><div style="display:flex;justify-content:space-between;gap:16px;align-items:center;flex-wrap:wrap"><div><h3 style="margin-bottom:6px">ChatGPT</h3><p class="muted" style="margin:0">OAuth 2.0 + PKCE · refresh tokens · MCP</p></div><span class="badge online">Ready</span></div><div class="field"><label>MCP Server URL</label><input id="mcpUrl" class="input" readonly value="https://lucas.autozon.xyz/mcp"></div><p class="muted">In ChatGPT web, enable Developer mode, create a custom app, enter this MCP URL, choose OAuth, and Scan Tools. ChatGPT will open the Lucas authorization page automatically.</p><div class="toolbar"><button class="btn primary" onclick="connectChatGPT()">Connect ChatGPT</button><button class="btn secondary" onclick="copyMcpUrl()">Copy MCP URL</button></div></div></div><div id="logs" class="view hidden"><div class="top"><div><h2>Activity Logs</h2><p class="muted">Only activity belonging to your account is shown.</p></div></div><div class="log-filter"><input id="logAction" class="input" placeholder="Action filter"><input id="logTarget" class="input" placeholder="Project / target"><button class="btn secondary" onclick="loadLogs()">Filter</button></div><div id="logTable"></div></div><div id="account" class="view hidden"><div class="top"><div><h2>Account & Security</h2><p class="muted">Authentication and connector security information.</p></div></div><div class="card"><h3 id="accountName"></h3><p id="accountEmail"></p><p class="muted">Authentication provider: <span id="accountProvider"></span></p><p class="muted">All AI-to-Windows traffic is relayed through this VPS Gateway. Windows nodes do not require a public inbound port.</p></div></div></main></div>
...
async function connectChatGPT(){await copyMcpUrl();toast('MCP URL copied — opening ChatGPT Apps');setTimeout(()=>window.open('https://chatgpt.com/apps','_blank','noopener'),350)}
